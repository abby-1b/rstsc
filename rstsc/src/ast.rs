...
pub enum ImportDefinition {
  ...
  Individual {
    source: SrcMapping,
    parts: SmallVec<IndividualImport>,
  },
  /// Used in `import '...'`
...
pub enum ASTNode {
  ...
  StatementTryCatchFinally {
    inner: Box<TryCatchFinally>,
  },

  FunctionDefinition {
    inner: Box<FunctionDefinition>,
  },
  ArrowFunctionDefinition {
    inner: Box<ArrowFunctionDefinition>,
  },

  ClassDefinition {
    inner: Box<ClassDefinition>,
  },
  ...
  Dict {
    properties: SmallVec<ObjectProperty>,
  },
  ...
  ExprRegexLiteral {
    inner: Box<ExprRegexLiteral>,
  },
  ExprTemplateLiteral {
    inner: Box<ExprTemplateLiteral>,
  },
  ExprBoolLiteral {
    value: bool,
  },
  ...
  EnumDeclaration {
    inner: Box<EnumDeclaration>,
  },
  InterfaceDeclaration {
    inner: Box<InterfaceDeclaration>,
  },

  /// `namespace X { ... }`
  NamespaceDeclaration {
    inner: Box<NamespaceDeclaration>,
  },
...
impl ASTNode {
  ...
  pub fn apply_modifiers(&mut self, new_modifiers: ModifierList) -> Result<(), CompilerError> {
    ...
    match self {
      ...
      DeclareNamespace { inner } => {
        inner.modifiers = new_modifiers;
      }
      _ => {
        return Err(CompilerError::new_static(format!(
