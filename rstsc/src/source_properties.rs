...
impl<'a> SourceProperties<'a> {
  ...
  pub fn str_src<'b>(&'b self, mapping: SrcMapping) -> &'a str
  where
    'a: 'b,
  {
  ...
  fn print_ast_node_debug(&self, node_index: ASTIndex, indent_level: usize, output: &mut String) {
    ...
    match node {
      ...
      VariableDeclaration {
        ...
        output.push_str(&format!(
          "{}VariableDeclaration {}{} ({} definitions)\n",
          indent,
          modifiers_str,
          def_type.emit(),
          defs.len()
      ...
      InterfaceDeclaration { inner } => {
        ...
        output.push_str(&format!(
          "{}  Type: [simplified type representation]\n",
          indent
        ));
      }
      ...
      StatementImport { inner } => match &**inner {
        ...
        ImportDefinition::Individual { source, parts } => {
          output.push_str(&format!(
            "{}StatementImport Individual ({} parts)\n",
            indent,
            parts.len()
          ));
          output.push_str(&format!("{}  Source: {}\n", indent, self.str_src(*source)));
      ...
      StatementExport { inner } => {
        output.push_str(&format!(
          "{}StatementExport ({} specifiers)\n",
          indent,
          inner.specifiers.len()
        ));
      ...
      ExprRegexLiteral { inner } => {
        output.push_str(&format!(
          "{}ExprRegexLiteral: /{}/{}\n",
          indent,
          self.str_src(inner.pattern),
          self.str_src(inner.flags)
        ));
      }
  ...
  fn print_class_member_debug(
    ...
    match member {
      ...
      ClassMember::Method(method, getter_setter) => {
        ...
        let name = method
          .name
          .map(|n| self.str_src(n))
          .unwrap_or("<anonymous>");
        output.push_str(&format!(
          "{}Method {}{}\n",
          indent,
          getter_setter_str,
          if getter_setter_str.is_empty() {
            ""
          } else {
            " "
          }
  ...
  fn print_object_property_debug(
    &self,
