...
fn main() {
  ...
  println!(
    "average: {: >12?} ({: >8?} loc/s)",
    mean,
    ((Duration::from_secs(1).as_secs_f64() / (mean).as_secs_f64()) * lines as f64).round() as usize
  );
  println!(
    "middle : {: >12?} ({: >8?} loc/s)",
    median,
    ((Duration::from_secs(1).as_secs_f64() / (median).as_secs_f64()) * lines as f64).round()
      as usize
  );
  println!(
    "min    : {: >12?} ({: >8?} loc/s)",
    durations[0],
    ((Duration::from_secs(1).as_secs_f64() / (durations[0]).as_secs_f64()) * lines as f64).round()
      as usize
  );
  println!(
    "max    : {: >12?} ({: >8?} loc/s)",
    durations[durations.len() - 1],
    ((Duration::from_secs(1).as_secs_f64() / (durations[durations.len() - 1]).as_secs_f64())
      * lines as f64)
      .round() as usize
  );
}
